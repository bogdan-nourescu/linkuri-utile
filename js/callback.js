...
function getLista() {
    ajax("GET", "https://test-d2273.firebaseio.com/.json", undefined, function (raspuns) {
        lista = raspuns;
        draw();
    });
}
...
function del(idx) {
    if (confirm(`are you sure you want to delete ${lista[idx].name}?`)) {
        ajax("DELETE", `https://test-d2273.firebaseio.com/${idx}.json`, undefined, function () {
            getLista();
        });
    }
}
...
function add() {
    ...
    */
    ...
    if (idxEdit === null) {
        ...
        ajax("POST", `https://test-d2273.firebaseio.com/.json`, JSON.stringify(el), getLista);
    } else {
        ajax("PUT", `https://test-d2273.firebaseio.com/${idxEdit}.json`, JSON.stringify(el), function () {
            getLista();
            idxEdit = null;
        });
    }
