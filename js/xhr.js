...
function del(idx) {
    if (confirm(`are you sure you want to delete ${lista[idx].name}?`)) {
        ...
        xhttp.open("DELETE", `https://test-d2273.firebaseio.com/${idx}.json`, true);
        xhttp.send();
    }
}
...
function add() {
    ...
    */
    ...
    if (idxEdit === null) {
        ...
        xhttp.open("POST", `https://test-d2273.firebaseio.com/.json`, true);
        xhttp.send(JSON.stringify(el));
    } else {
        const xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
        ...
        xhttp.open("PUT", `https://test-d2273.firebaseio.com/${idxEdit}.json`, true);
        xhttp.send(JSON.stringify(el));
    }
}
