...
function getLista() {
    ajax("GET", "https://test-d2273.firebaseio.com/.json")
...
function del(idx) {
    if (confirm(`are you sure you want to delete ${lista[idx].name}?`)) {
        ajax("DELETE", `https://test-d2273.firebaseio.com/${idx}.json`)
            .then(function (raspuns) {
                getLista();
            })
            .catch(function (err) {
...
function add() {
    ...
    */
    ...
    if (idxEdit === null) {
        ...
        ajax("POST", `https://test-d2273.firebaseio.com/.json`, JSON.stringify(el))
            .then(getLista)
            .catch(function (err) {
                console.error(err);
            });
    } else {
        ajax("PUT", `https://test-d2273.firebaseio.com/${idxEdit}.json`, JSON.stringify(el))
            .then(function () {
                getLista();
                idxEdit = null;
                document.querySelector('[type="submit"]').value = "Add";
