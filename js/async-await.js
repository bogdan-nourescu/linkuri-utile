let lista = {};
let idxEdit = null;
const complexityColors = ["yellow", "orange", "red", "darkred", "black"];
...
async function getLista() {
    lista = await ajax("GET", "https://test-d2273.firebaseio.com/.json");
    draw();
}
...
async function del(idx) {
    if (confirm(`are you sure you want to delete ${lista[idx].name}?`)) {
        await ajax("DELETE", `https://test-d2273.firebaseio.com/${idx}.json`);
        await getLista();
    }
}
...
async function add() {
    ...
    */
    ...
    if (idxEdit === null) {
        ...
        await ajax("POST", `https://test-d2273.firebaseio.com/.json`, JSON.stringify(el));
        await getLista();
    } else {
        await ajax("PUT", `https://test-d2273.firebaseio.com/${idxEdit}.json`, JSON.stringify(el));
        await getLista();
        idxEdit = null;
        document.querySelector('[type="submit"]').value = "Add";
    }
